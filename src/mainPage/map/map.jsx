...
class MapPannel extends React.Component {
  ...
  onMouseMove(map, evt) {
    if (this.state.from_marker.enabled && !this.state.from_marker.placed) {
      ...
      markerGeojson.features[1].geometry.coordinates = [
        evt.lngLat.lng,
        evt.lngLat.lat
      ];
      map.getSource("point").setData(markerGeojson);
            ...
            <Label as="a" basic pointing="left">
              {from_marker.placed &&
                Math.round(from_marker.lngLat.lat * 10000) / 10000 +
                  ", " +
                  Math.round(from_marker.lngLat.lng * 10000) / 10000}
            </Label>
          </Button>
            ...
            <Label as="a" basic pointing="left">
              {to_marker.placed &&
                Math.round(to_marker.lngLat.lat * 10000) / 10000 +
                  ", " +
                  Math.round(to_marker.lngLat.lng * 10000) / 10000}
            </Label>
          </Button>

          <Button
            disabled={
              !(this.state.from_marker.lngLat && this.state.to_marker.lngLat) ||
